...
| Windows 11 (x64) | **most of it** | Console selection verified live from Windows: four selections on a real WING produced four correct rack commands. The rack host side too: discovered, connected, inventory read, racks opened on command, health received, and reconnected on its own. What is still untried there: the console's USER buttons, and MIDI to the rack host, which needs a loopMIDI port. |
